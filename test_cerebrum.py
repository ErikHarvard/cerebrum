...
print("== the ledger: every input and retrieval on record, rendered into the vault as ARCHIVUM CEREBRI ==")
tmp25 = tempfile.mkdtemp(prefix="cerebrum-ledger-")
try:
    v, ocfg = organ_vault(tmp25); os.makedirs(os.path.join(v, "2 AREAS", "META"))
    cfg = dict(ocfg, registry="2 AREAS/META/REG.md", archivum="2 AREAS/META/ARCHIVUM.md", meta_dirs=["2 AREAS/META"], law="2 AREAS/META/LAW.md")
    open(os.path.join(v, "2 AREAS/META/LAW.md"), "w").write("# Law\n\n**Where.** `2 AREAS/META/`\n")
    real_dir = AR.LEDGER_DIR; AR.LEDGER_DIR = os.path.join(tmp25, "ledger"); real_state = C.STATE; C.STATE = tmp25
    try:
        AR.ledger("intake", event="capture", note="# INBOX/X.md", title="X", source="a test", polished=True, chars=10)
        ...
        check("an archivum edited by hand is stale", not REG.fresh(v, cfg))
    finally:
        AR.LEDGER_DIR = real_dir; C.STATE = real_state
finally:
